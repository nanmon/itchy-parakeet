...
public class Cookiefloat {
    ...
    private static int POSTFIX[] = new int[] {
            R.plurals.million, R.plurals.billion, R.plurals.trillion, R.plurals.quadrillion,
            R.plurals.quintillion, R.plurals.sextillion, R.plurals.septillion,
            R.plurals.octillion, R.plurals.nonillion, R.plurals.decillion,
            R.plurals.undecillion, R.plurals.duodecillion, R.plurals.tredecillion,
            R.plurals.quattuordecillion, R.plurals.qunindecillion
    };
    ...
    public static String format(double f, boolean round){
        if(f>=1000.0){
            String mod;
            if(f<1000000.0){
                if(round || f%1==0)
                    mod = String.format(",%03d", (int)f%1000);
                else mod = String.format(",%05.1f", f%1000);
                return (int)f/1000 + mod;
            }
            int log1000 = (int)Math.log10(f)/3;
            double f2 = f/Math.pow(1000, log1000-1);
            if((int)f2%1000 == 0) return (int)f2/1000
                    +" "+ context.getResources().getQuantityString(POSTFIX[log1000-2], (int)f2/1000);
            if((int)f2%100==0)
                mod = String.format(".%d",(int)(f2%1000)/100);
            else if((int)f2%10==0)
                mod = String.format(".%02d", (int)(f2%1000)/10);
            else mod = String.format(".%03d", (int)(f2%1000));
            return  (int)f2/1000 + mod
                    +" "+ context.getResources().getQuantityString(POSTFIX[log1000-2], 0);
        }
        if(round || f%1==0)
            return String.format("%d", (int)f);
        return String.format("%.1f", f);

        /*String r = String.format("%f", f);
        int p = r.indexOf('.');
        ...
        if(rl>3){
            if(rl<=6){
                r = r.substring(0, rl-3)+"," +r.substring(rl-3);
                return r.substring(0, round?p+1:p+3);
            }
            //if(p!=-1) r = r.substring(0, p);
            int res = rl%3;
            if(res==0) res=3;
            r =  r.substring(0, res)+"."+r.substring(res, 3+res); //+" "+POSTFIX[(rl-res)/3-2];  //"e"+(rl-res);
            for(int i=0; i<4 && r.endsWith("0") || r.endsWith("."); ++i) r = r.substring(0, r.length()-1);
            double aFloat = Double.parseDouble(r);
            return r + " "+context.getResources().getQuantityString(POSTFIX[(rl-res)/3-2], (int)aFloat==aFloat?(int)aFloat:0);
        }

        return r.substring(0, round?p:p+2);*/
    }

    public static String format(long f){
        return Cookiefloat.format((double)f, true);
    }
}
